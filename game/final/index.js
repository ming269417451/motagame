    ...
    function gameLogic({context, allSpriteImg, heroImg}) {
        ...
        document.onkeyup = function (event) {
                ...
                if (keyMap[event.keyCode]) {
                    hero.walk(keyMap[event.keyCode], function (newPos) {
                        const element = mapManager.getElement(newPos);
                            console.log('round!!!', element);
                        if (element && element.attackAble) {
                            function fight() {
                                element.attacked(hero);
                                hero.attacked(element);
                                mapManager.flush();
                                if (hero.checkAlive() && element.checkAlive()) {
                                    setTimeout(() => {
                                        fight();
                                    }, 500);
                                }
                        ...
                        return mapManager.judgeIn(newPos);
                    });
                    mapManager.flush();
                }
        };
